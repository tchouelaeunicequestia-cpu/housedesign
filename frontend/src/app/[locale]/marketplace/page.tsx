    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between">
      <div>
        <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-10 shadow-md">
          <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <nav className="flex items-center gap-6 text-sm font-medium text-slate-300">
              ...
              <Link href="/marketplace" className="text-cyan-400 font-bold">{t('nav.marketplace')}</Link>
            </nav>
        ...
        <section className="bg-slate-900 text-white py-16 px-6 text-center border-b border-slate-800">
          <div className="max-w-3xl mx-auto">
            ...
            <h2 className="text-4xl font-extrabold mb-3 text-white">{t('hero.title')}</h2>
        ...
        <main className="max-w-6xl mx-auto px-6 py-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div className="flex items-center gap-3">
              <h3 className="text-2xl font-bold text-white">{t('availableAssets')}</h3>
          ...
          {isLoading && (
            <div className="text-cyan-400 font-medium py-12 text-center">{t('loading')}</div>
          )}
          ...
          {viewMode === 'map' ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 h-[500px] flex flex-col items-center justify-center relative overflow-hidden shadow-xl">
              ...
              <h4 className="text-lg font-bold text-white">{t('map.title')}</h4>
              <p className="text-sm text-slate-400 text-center max-w-md mt-1">
                {t('map.description', { count: filteredAssets?.length || 0 })}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredAssets?.map((asset: any) => {
                // Mapped to backend property 'mediaUrls'
                  ...
                  >
                    ...
                    <div className="p-6 bg-slate-950 border-t border-slate-800/80 flex flex-col gap-4">
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="text-[10px] uppercase tracking-wider text-slate-500 font-bold block">{t('askingPrice')}</span>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-white">{t('docsModal.title')}</h3>
              </div>
              <button
            ...
            <div className="space-y-3">
              <p className="text-xs text-slate-400">
                {t('docsModal.description', { title: selectedAssetForDocs.title })}
              </p>
                ...
                {selectedAssetForDocs.documentUrls?.length > 0 ? (
                  selectedAssetForDocs.documentUrls.map((doc: string, idx: number) => (
                    <div key={idx} className="flex justify-between items-center text-xs text-slate-300 py-2 border-b border-slate-900 last:border-none">
                      ...
                      <span className="text-emerald-400 font-semibold bg-emerald-950 px-2 py-0.5 rounded">{t('docsModal.verifiedSecure')}</span>
                    </div>
                  ))
                ) : (
                  <p className="text-xs text-slate-500 py-2 text-center">{t('docsModal.noDocuments')}</p>
                )}
              </div>
            </div>
            <button
      ...
      {selectedAssetForOffer && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-white">{t('offerModal.title', { title: selectedAssetForOffer.title })}</h3>
              <button
            ...
            <form onSubmit={handleOfferSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">{t('offerModal.priceLabel')}</label>
                <input
                  type="number"
                  required
                  value={offerAmount}
                  onChange={(e) => setOfferAmount(e.target.value)}
                  placeholder={t('offerModal.pricePlaceholder', { price: selectedAssetForOffer.price })}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">{t('offerModal.messageLabel')}</label>
                <textarea
                  rows={3}
                  value={offerMessage}
              ...
              >
                {isSubmittingOffer ? t('offerModal.submitting') : t('offerModal.submitButton')}
              </button>
            </form>
          </div>
        </div>
